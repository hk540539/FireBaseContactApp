...
export default class AddNewContact extends Component {
    ...
    // create and save contact to firebase
		...
		if (
			...
			// uploading image to storage asyncly
			...
			// uploading contact data to database asyncly with uploaded image storage url
	...
	uploadImageAsync = async (uri, storageRef) => {
		...
		//upload part to firebase
		const ref = storageRef.child('ContactImages').child(uuid.v4() + '.' + fileExtenstion);
